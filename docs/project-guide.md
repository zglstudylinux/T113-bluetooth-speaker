...
### ✅ M7：M6 板上回归通过 + 两个 UI bug 修复（已完成 2026-08-31）

**背景**：M6 架构重构一次性完成后上板回归。杀掉板上原运行的小智 LVGL 程序，
`deploy.sh` 部署新架构产物。回归通过：手机自动回连 → 播放真实曲目（AVRCP 元数据/
进度实时刷新）→ 唱盘旋转（fb 抓帧两帧比对差异 12055 像素确认）→ hci0
`UP RUNNING PSCAN ISCAN` 无回归。同时发现两个 UI bug，本次一并修复。

**Bug 1：长歌名压住进度条**

- **现象**：《无赖 (电影《龙咁威II之皇母娘娘呢》主题曲)》约 30 字换行到 3 行，
  歌名 label 侵入歌手行和进度条区域。
- **根因**：歌名 label 只 `lv_obj_set_width(440)` 不设高度，`LV_LABEL_LONG_WRAP`
  下高度随内容无限增长（LVGL 的 label 默认高度就是内容高）。之前 M5b 调试时
  碰巧只遇到 1~2 行的歌名，没暴露。
- **修复**（`ui/themes/liquidglass/ui_liquidglass.c` 新增 `apply_song()`）：
  每次设歌名先按 `LV_SIZE_CONTENT` 不限行数测量真实高度（`lv_obj_update_layout`
  强制布局），再按行数分三档重排——**单行**保持设计稿位（top=384 视觉中心 420）、
  显示歌手行；**两行**整体上移使块中心仍 ≈420、隐藏歌手行（歌手行 top=444 会撞
  第二行）；**≥3 行**限高两行走 `LV_LABEL_LONG_DOT` 截断成两行+"…"。
  函数无跨调用状态，每次全量重算。
- **LVGL 知识点**：`LONG_DOT` 只在"固定高度 + 文本超两行"时才截断（只设宽度时
  label 高度跟着内容走，永远不超 → 从不截断）；且模式切换/设新文本时
  `lv_label_refr_text` 会先 `revert_dots` 还原被截的原文再重算，我们直接设
  long_mode + text 的调用顺序天然正确。

**Bug 2：暂停态点上一首/下一首，声音恢复播放了但图标不变**

- **现象**：手机暂停后点板子切歌按钮，手机恢复播放（切歌=播放，AVRCP 协议行为），
  但板子图标停在 ▶，要等 ~1s 或根本不等回调。
- **根因**：M2/M3 起乐观更新只加在播放按钮（`btn_play_cb`）上，`btn_prev_cb`/
  `btn_next_cb` 只发命令不动图标。而 AVRCP play_state 回调是手机周期性上报的
  （§8.1 老坑），状态变化不保证立刻上报。
- **修复**：切歌按钮补同款乐观更新——暂停态（`!g_playing`）点击瞬间图标切 ⏸、
  `g_playing=true`（唱盘同步开转）；若手机实际没恢复播放（少数手机切歌保持暂停），
  ~1s 后 AVRCP 回调自动校正回来。乐观更新的"先斩后奏+回调校正"模式现在三个
  按钮一致。

**验证结果**（用户上板确认）：
- ✅ M6 回归：配对/连接/播放出声/元数据/进度/唱盘旋转全通过
