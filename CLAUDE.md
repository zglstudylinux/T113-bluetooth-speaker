...
# 编译目标板（增量；CMake 自动跟踪 lv_conf.h 头文件依赖）
cmake -B build-cmake -DCMAKE_TOOLCHAIN_FILE=cmake/toolchain/openwrt-armhf.cmake   # 仅首次
cmake --build build-cmake -j$(nproc)

# 部署到板子并启动（adb push + start-stop-daemon）
./scripts/deploy.sh

# host 自测（可移植层：OSAL 队列 + sim 整链路，无需工具链/板子）
cmake -B build-host && cmake --build build-host -j && ctest --test-dir build-host --output-on-failure
```
...
1. **工具链 ABI 是项目基石**：必须用 `toolchain-sunxi-glibc-gcc-830`（gcc 8.3，**armhf 硬浮点**），与板上 rootfs（glibc 2.29 armhf）和 `libbtmg.so` 匹配。**不能用** SDK 默认的 linaro 5.3.1 `arm-linux-gnueabi`（软浮点）。CMake toolchain 文件 `cmake/toolchain/openwrt-armhf.cmake` 强制指向 `./toolchain/bin/arm-openwrt-linux-gcc`。
...
5. **host 环境不能构建 `bt_speaker` 目标**（vendor ARM .so 在 host 链接报 wrong format）——CMakeLists 检测裸名 gcc/cc/clang 时只编 osal_test/sim_loop_test 并 return；给目标板构建必须带 toolchain 文件。
...
## 代码结构（big picture，M6 分层架构）

```
core/player_types.h        # 唯一数据契约：player_event_t（208B 定长值类型；-1/空串=不更新）
osal/                      # OS 抽象：队列（16槽满丢最旧）/mutex/time/log；osal_posix.c（Linux）
services/
├── player_backend.h       # 业务接口：init(emit)/query_state/cmd
├── btmg/btmg_player.c     # btmanager 实现：preinit→A2DP Sink|AVRCP→回调组事件 emit 入队
└── sim/sim_player.c       # 模拟播放器（剧本吐事件；host 开发/CI 用）
ui/
├── ui_backend.h           # UI 接口：init(ui_env_t)/on_event/deinit
└── themes/liquidglass/    # D1 液态玻璃主题：theme.h（色板/素材/布局）+ 绘制/唱盘旋转/乐观更新
ports/                     # 板级：lv_port_disp(fb)/indev(evdev)/font(FreeType) + main_linux.c
apps/app_player.c          # 组装层：建队列 → UI init → 后端 init → lv_timer 33ms drain
```
...
**图片加载**：bg.png/disc.png（`LV_FS_POSIX_LETTER 'S'`，`ports/…/theme.h` 定义路径）；启动时 `access()` 探测降级。换图同名覆盖重启 app 即生效。

## LVGL 配置要点（ports/lv_conf.h）
