 ...
 *   3. init ui_backend_liquidglass，lv_timer 33ms drain：事件 → ui->on_event
 *
 * drain 在 LVGL 线程消费，事件栈上直传（定长值拷贝，零堆分配）——
 * 替代旧架构的 lv_async_call + malloc 消息通道。
 */
#include "lvgl/lvgl.h"
...
static const ui_backend_t *g_ui;
...
int app_player_start(const char *alias, const ui_backend_t *ui, const ui_env_t *env)
{
    ...
    g_ui = ui;
    if (g_ui->init(env) != 0) {
        fprintf(stderr, "app_player: ui init fail\n");
...
/* UI 按钮 → 播放控制（LVGL 线程直调，乐观更新在主题侧，此路只发命令） */
